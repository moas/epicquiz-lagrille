...
<div class="backdrop" role="presentation" onclick={(event) => event.currentTarget === event.target && !isSaving && onclose()}>
	<dialog class="dialog" open aria-labelledby="dispatch-title">
		...
		{:else}<section class="assignment-list" aria-label="Prévisualisation des attributs">{#each shuffledAttributes as item}<article><div><span class:steal={item.attribute.type === 'steal'} class="kind">{item.attribute.type === 'steal' ? 'VOL' : 'LOT'}</span><strong>{item.attribute.type === 'steal' ? 'Vol' : item.attribute.name}</strong></div><span class="target">Case {item.cell.name}</span></article>{/each}</section>{/if}
