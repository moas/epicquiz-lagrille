<script lang="ts">
	...
	let { episode, configurationLocked = false, onclose, onconfigured }: { episode: Episode; configurationLocked?: boolean; onclose: () => void; onconfigured: (episode: Episode) => void } = $props();
	...
	let maxAttrsPerCell = $state(existing?.max_attrs_per_cell ?? 1);
	...
	function generateDistribution() {
		formError = '';
		distributionError = '';
		if (!Number.isInteger(rows) || !Number.isInteger(columns) || rows < 1 || columns < 1 || !Number.isInteger(emptyCellCount) || emptyCellCount < 0 || emptyCellCount >= cellCount) {
			formError = 'Définissez d’abord des dimensions et un nombre de cases vides valides.';
			return;
		}
		if (playableCells < 20 || playableCells > 100) {
			formError = `Impossible de répartir exactement 100 points sur ${playableCells} cases jouables. Il faut entre 20 et 100 cases.`;
			return;
		}

		const scores = Array.from({ length: playableCells }, () => 1);
		let pointsToAllocate = 100 - playableCells;
		while (pointsToAllocate > 0) {
			const candidates = scores.reduce<number[]>((indexes, score, index) => score < 5 ? [...indexes, index] : indexes, []);
		...
		distribution = Object.fromEntries(
			Array.from({ length: 5 }, (_, index) => [String(index + 1), scores.filter((score) => score === index + 1).length]),
		);
		validate();
	}
	...
	function validate() {
		...
		if (!Number.isInteger(maxAttrsPerCell) || maxAttrsPerCell < 1) {
			formError = 'Chaque case doit pouvoir accepter au moins un attribut spécial.';
			return false;
		}
	...
	async function save() {
		...
		try {
			const gridConfig: GridConfig = {
				...
				max_attrs_per_cell: maxAttrsPerCell,
...
<div class="backdrop" role="presentation" onclick={(event) => event.currentTarget === event.target && !isSaving && onclose()}>
	<dialog class="dialog" open aria-labelledby="grid-config-title">
		...
		{#if configurationLocked}<p class="locked-notice" role="status">Configuration de la grille verrouillée : les challenges ont été confirmés. Les lots restent configurables jusqu’à leur propre tirage.</p>{/if}
		<form onsubmit={(event) => { event.preventDefault(); void save(); }}>
			<div class="dimensions"><label for="grid-rows">Lignes<input id="grid-rows" type="number" min="1" max="99" bind:value={rows} disabled={isSaving || configurationLocked} onblur={validate} /></label><label for="grid-columns">Colonnes<input id="grid-columns" type="number" min="1" max="99" bind:value={columns} disabled={isSaving || configurationLocked} onblur={validate} /></label><label for="grid-empty">Cases vides<input id="grid-empty" type="number" min="0" max={Math.max(0, cellCount - 1)} bind:value={emptyCellCount} disabled={isSaving || configurationLocked} onblur={validate} /></label></div>
			<section class="distribution" aria-labelledby="points-title"><div class="distribution-heading"><div><h3 id="points-title">Répartition par points</h3><p><strong>{distributedCells} / {playableCells} cases attribuées</strong> · maximum : <strong>{totalPoints} / 100 points</strong>.</p></div>{#if !configurationLocked}<button class="generate" type="button" disabled={isSaving} onclick={generateDistribution}>Générer une répartition</button>{/if}</div><div class="point-inputs">{#each ['1', '2', '3', '4', '5'] as points}<label for={`points-${points}`}><span>{points} pts</span><input id={`points-${points}`} type="number" min="0" bind:value={distribution[points]} disabled={isSaving || configurationLocked} onblur={validate} /></label>{/each}</div>{#if distributionError}<p class="field-error" role="alert">{distributionError}</p>{/if}</section>
			<section class="attributes" aria-labelledby="attributes-title"><div><p class="section-label">Répartition</p><h3 id="attributes-title">Capacité des cases</h3><p>Définissez le nombre maximal d’attributs qui peut être attribué à une même case.</p></div><label class="attribute-limit" for="max-attrs-per-cell"><span class="control-label">Maximum par case</span><input id="max-attrs-per-cell" type="number" min="1" bind:value={maxAttrsPerCell} disabled={isSaving || configurationLocked} onblur={validate} /></label></section>
			{#if formError}<p class="form-error" role="alert">{formError}</p>{/if}
			<div class="actions">{#if configurationLocked}<button class="submit" type="button" onclick={onclose}>Fermer</button>{:else}<button class="cancel" type="button" disabled={isSaving} onclick={onclose}>Annuler</button><button class="submit" type="submit" disabled={isSaving}>{isSaving ? 'Enregistrement…' : 'Enregistrer la grille'}</button>{/if}</div>
		</form>
	</dialog>
</div>

<style>
	.backdrop { position:fixed; z-index:20; inset:0; display:grid; place-items:center; padding:1rem; background:rgba(2,6,23,.7); backdrop-filter:blur(6px); }.dialog { width:min(100%,40rem); max-height:calc(100dvh - 2rem); overflow:auto; border:1px solid #475569; border-radius:1rem; padding:clamp(1.25rem,4vw,2rem); background:#1e293b; color:#f8fafc; box-shadow:0 1.5rem 5rem rgba(0,0,0,.42); }.heading { display:flex; justify-content:space-between; gap:1rem; }.eyebrow,.section-label { margin:0 0 .45rem; color:#c4b5fd; font-size:.7rem; font-weight:800; letter-spacing:.12em; text-transform:uppercase; }.heading h2 { margin:0; color:#f8fafc; font-size:1.55rem; letter-spacing:-.045em; }.close { width:2.75rem; height:2.75rem; flex:none; border:1px solid #475569; border-radius:.65rem; background:transparent; color:#cbd5e1; cursor:pointer; font-size:1.6rem; }.close:hover { border-color:#94a3b8; color:#f8fafc; }.intro { margin:1rem 0 1.5rem; color:#cbd5e1; font-size:.85rem; line-height:1.55; }.intro strong { color:#ddd6fe; font-weight:650; }.locked-notice { margin:-.7rem 0 1.5rem; border:1px solid rgba(245,158,11,.5); border-radius:.65rem; padding:.7rem .8rem; background:rgba(245,158,11,.1); color:#fde68a; font-size:.78rem; line-height:1.45; }.dimensions,.point-inputs { display:grid; grid-template-columns:repeat(3,minmax(0,1fr)); gap:.75rem; }.dimensions label,.point-inputs label { color:#f8fafc; font-size:.8rem; font-weight:750; }.dimensions input,.point-inputs input,.attribute-limit input { width:100%; min-height:2.7rem; margin-top:.45rem; border:1px solid #475569; border-radius:.55rem; padding:0 .65rem; background:#0f172a; color:#f8fafc; font:inherit; }.dimensions input:focus,.point-inputs input:focus,.attribute-limit input:focus { border-color:#a78bfa; outline:3px solid rgba(167,139,250,.24); }.dimensions input:disabled,.point-inputs input:disabled,.attribute-limit input:disabled { border-color:#334155; background:#17213a; color:#94a3b8; opacity:1; }.distribution,.attributes { margin-top:1.5rem; border:1px solid #334155; border-radius:.8rem; padding:1rem; background:#17213a; }.distribution-heading { display:flex; align-items:start; justify-content:space-between; gap:1rem; }.distribution h3,.attributes h3 { margin:0 0 .25rem; color:#f8fafc; font-size:.95rem; }.distribution p,.attributes>div:first-child>p:last-child { margin:0 0 1rem; color:#94a3b8; font-size:.78rem; line-height:1.45; }.generate { min-height:2.7rem; flex:none; border:1px solid #7c3aed; border-radius:.55rem; padding:.5rem .7rem; background:rgba(124,58,237,.16); color:#ddd6fe; cursor:pointer; font:inherit; font-size:.76rem; font-weight:800; }.generate:hover { background:rgba(124,58,237,.28); }.point-inputs label { display:flex; align-items:center; justify-content:space-between; gap:.5rem; }.point-inputs input { width:4.4rem; margin:0; }.attributes { display:flex; flex-wrap:wrap; align-items:end; justify-content:space-between; gap:1rem; }.attributes>div:first-child { min-width:min(100%,16rem); flex:1 1 17rem; }.attributes>div:first-child>p:last-child { margin-bottom:0; }.attribute-limit { flex:0 0 8rem; color:#f8fafc; font-size:.7rem; font-weight:750; }.attribute-limit input { min-height:2.7rem; margin:.35rem 0 0; }.control-label { display:block; overflow:hidden; color:#cbd5e1; font-size:.68rem; font-weight:750; line-height:1rem; text-overflow:ellipsis; white-space:nowrap; }.field-error,.form-error { margin:1rem 0 0; color:#fecaca; font-size:.8rem; line-height:1.45; }.actions { display:flex; justify-content:flex-end; gap:.7rem; margin-top:1.5rem; }.actions button { min-height:2.8rem; border-radius:.6rem; padding:.7rem 1rem; cursor:pointer; font:inherit; font-size:.85rem; font-weight:750; }.cancel { border:1px solid #475569; background:transparent; color:#cbd5e1; }.submit { border:1px solid #7c3aed; background:#7c3aed; color:#fff; }.cancel:hover { border-color:#94a3b8; color:#f8fafc; }.submit:hover { background:#8b5cf6; }.actions button:disabled,.close:disabled,.generate:disabled { cursor:not-allowed; opacity:.6; }.close:focus-visible,.actions button:focus-visible,.generate:focus-visible { outline:3px solid #a78bfa; outline-offset:3px; } @media (max-width:520px) { .dimensions,.point-inputs { grid-template-columns:1fr; }.distribution-heading { align-items:stretch; flex-direction:column; }.attribute-limit { flex-basis:100%; }.generate { width:100%; }.point-inputs label { min-height:2.75rem; }.actions { flex-direction:column-reverse; }.actions button { width:100%; } }
</style>
