<script lang="ts">
	...
	let selectedQuestionCount = $state(0);
	...
	const questionCountGap = $derived(expectedQuestionCount - selectedQuestionCount);
	...
	const hasLevelMismatch = $derived(levelRequirements.some((requirement) => requirement.gap !== 0));
	const canDispatchChallenges = $derived(expectedQuestionCount > 0 && questionCountGap === 0 && !hasLevelMismatch);
	const activeAttributeCount = $derived(specialAttributes.filter((attribute) => attribute.is_active).length);
	...
	async function loadAttributes() {
		specialAttributes = (await Promise.all([getStealAttributes(params.id), getPrizeAttributes(params.id)])).flat();
	}

	onMount(async () => {
		try {
			const [loadedEpisode, questionPool, questions, attributes] = await Promise.all([
			...
			selectedQuestionCount = questionPool.selected_count;
...
<main>
	...
	{:else if episode}
		<header><div><p>Épisode · {episode.title}</p><h1>Grille</h1><span>{gridLabel(episode)}</span></div><button onclick={() => isEditing = true}>{episode.metadata.grid_config ? 'Modifier la configuration' : 'Configurer la grille'}</button></header>
		{#if episode.metadata.grid_config}
			...
			<section class="configuration-cards" aria-label="Préparer les contenus de la grille">
				<article class="configuration-card featured" class:blocked={!canDispatchChallenges && !challengesSaved}><span class="card-number">01</span><p class="eyebrow">Questions de la grille</p><h2>Challenges</h2><p>Associez les questions aux cases, prévisualisez un tirage aléatoire puis confirmez la répartition.</p>{#if challengesSaved}<div class="pool-status ready" role="status"><strong>Challenges enregistrés</strong><span>{preparedGrid?.cells.filter((cell) => cell.challenge_id).length} cases contiennent une question. Le tirage est verrouillé.</span></div>{:else}<div class:ready={canDispatchChallenges} class="pool-status" role="status"><strong>{selectedQuestionCount} / {expectedQuestionCount} questions retenues</strong>{#if canDispatchChallenges}<span>Le pool correspond à la grille : prévisualisation disponible.</span>{:else}<span>La distribution doit aussi correspondre à chaque niveau.</span><ul class="level-checks">{#each levelRequirements as requirement}<li class:match={requirement.gap === 0}><b>Niv. {requirement.level}</b><span>{requirement.selected} / {requirement.expected}</span>{#if requirement.gap > 0}<em>manque {requirement.gap}</em>{:else if requirement.gap < 0}<em>en trop {Math.abs(requirement.gap)}</em>{:else}<em>ok</em>{/if}</li>{/each}</ul>{#if questionCountGap > 0}<span>Il manque aussi {questionCountGap} question{questionCountGap === 1 ? '' : 's'} au total.</span>{:else if questionCountGap < 0}<span>Il y a aussi {Math.abs(questionCountGap)} question{Math.abs(questionCountGap) === 1 ? '' : 's'} en trop.</span>{/if}{/if}</div><button type="button" disabled={!canDispatchChallenges} onclick={() => isDispatchingChallenges = true}>{canDispatchChallenges ? 'Prévisualiser le tirage' : 'Distribution à compléter'} <span aria-hidden="true">→</span></button>{/if}</article>
				<article class="configuration-card"><span class="card-number">02</span><p class="eyebrow">Récompenses</p><h2>Lots</h2><p>Définissez les lots à attribuer aux joueurs au fil de la session.</p><button type="button" class="secondary" onclick={() => notice = 'La définition des lots sera disponible prochainement.'}>Définir les lots <span aria-hidden="true">→</span></button></article>
				<article class="configuration-card"><span class="card-number">03</span><p class="eyebrow">Mécaniques de jeu</p><h2>Attributs spéciaux</h2><p>Préparez les bonus, vols ou effets à appliquer aux cases de la grille.</p><div class:ready={activeAttributeCount > 0} class="attribute-status"><strong>{activeAttributeCount} attribut{activeAttributeCount === 1 ? '' : 's'} actif{activeAttributeCount === 1 ? '' : 's'}</strong><span>{attributesSaved ? 'Attribution enregistrée.' : challengesSaved ? 'Prêt pour un tirage aléatoire.' : 'Disponible après la répartition des challenges.'}</span></div>{#if attributesSaved}<button type="button" class="secondary" onclick={() => isManagingAttributes = true}>Voir les attributs <span aria-hidden="true">→</span></button>{:else if challengesSaved && activeAttributeCount > 0}<div class="dual-actions"><button type="button" class="secondary" onclick={() => isManagingAttributes = true}>Gérer</button><button type="button" onclick={() => isDispatchingAttributes = true}>Attribuer <span aria-hidden="true">→</span></button></div>{:else}<button type="button" class="secondary" onclick={() => isManagingAttributes = true}>Gérer les attributs <span aria-hidden="true">→</span></button>{/if}</article>
			</section>
			{#if notice}<p class="notice" role="status">{notice}</p>{/if}
			<section class="details"><div><small>Cases vides</small><strong>{episode.metadata.grid_config.empty_cell_count}</strong></div>{#each Object.entries(episode.metadata.grid_config.point_distribution) as [points, count]}<div><small>{points} points</small><strong>{count} cases</strong></div>{/each}</section>
		...
		{#if isEditing}<GridConfigurationDialog {episode} onclose={() => isEditing = false} onconfigured={(updated) => { episode = updated; isEditing = false; }} />{/if}
		{#if isManagingAttributes}<SpecialAttributesDialog episodeId={params.id} locked={attributesSaved} onclose={() => isManagingAttributes = false} onchange={(attributes) => specialAttributes = attributes} />{/if}
