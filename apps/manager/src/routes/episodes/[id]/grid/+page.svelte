	...
	{:else if episode}
		...
		{#if episode.metadata.grid_config}
			...
			<section class="configuration-cards" aria-label="Préparer les contenus de la grille">
				...
				<article class="configuration-card" class:blocked={challengesSaved && !attributesSaved && activeAttributeCount === 0}><span class="card-number">02</span><p class="eyebrow">Lots et mécaniques</p><h2>Attributs spéciaux</h2><p>Attribuez aléatoirement les lots et les vols définis dans la configuration de la grille.</p><div class:ready={activeAttributeCount > 0 || attributesSaved} class="attribute-status"><strong>{attributesSaved ? 'Attributs enregistrés' : `${activeAttributeCount} attribut${activeAttributeCount === 1 ? '' : 's'} actif${activeAttributeCount === 1 ? '' : 's'}`}</strong><span>{attributesSaved ? 'Le tirage est verrouillé.' : challengesSaved && activeAttributeCount > 0 ? 'Prêt pour un tirage aléatoire.' : challengesSaved ? 'Ajoutez des attributs depuis la configuration.' : 'Disponible après la répartition des challenges.'}</span></div>{#if attributesSaved}<button type="button" class="secondary" disabled>Attribution terminée</button>{:else}<button type="button" disabled={!challengesSaved || activeAttributeCount === 0} onclick={() => isDispatchingAttributes = true}>{challengesSaved && activeAttributeCount > 0 ? 'Prévisualiser le tirage' : 'Tirage indisponible'} <span aria-hidden="true">→</span></button>{/if}</article>
			</section>
		...
		{#if isEditing}<GridConfigurationDialog {episode} attributesLocked={attributesSaved} onclose={() => isEditing = false} onconfigured={(updated) => { episode = updated; isEditing = false; }} onattributeschange={(attributes) => specialAttributes = attributes} />{/if}
