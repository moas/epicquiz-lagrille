<script lang="ts">
	...
	async function resetQuestionFilters() {
		search = '';
		filterLevel = 'all';
		await loadQuestions(1);
	}
...
<main>
	...
	{:else if episode}
		<header><p>Épisode · {episode.title}</p><h1>Questions</h1><div class="question-counts" aria-label={`${questionPage?.selected_count ?? 0} questions sélectionnées sur ${questionPage?.eligible_count ?? 0} disponibles`}><span class="selected" title="Questions sélectionnées">{questionPage?.selected_count ?? 0}</span><span class="available" title="Questions disponibles">{questionPage?.eligible_count ?? 0}</span></div></header>
		...
		<section class="question-pool" aria-labelledby="pool-title">
			...
			{:else}<div class="pool-empty"><strong>{configs.length ? 'Aucune question ne correspond aux règles.' : 'La bibliothèque ne contient encore aucune question active.'}</strong><p>{configs.length ? 'Modifiez vos critères pour rendre des questions éligibles.' : 'Ajoutez des questions à la bibliothèque pour alimenter ce pool.'}</p></div>{/if}
